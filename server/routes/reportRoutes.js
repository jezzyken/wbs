...
router.get('/collections/daily', async (req, res) => {
  try {
    ...
    .populate({
      path: 'consumerId',
      select: 'firstName lastName middleName nameExtension accountNo fullAddress'
    })
    .populate({
      path: 'billId',
    ...
    const summary = {
      totalAmount: collections.reduce((sum, payment) => sum + payment.amount, 0),
      totalReceived: collections.reduce((sum, payment) => sum + payment.amountReceived, 0),
      totalChange: collections.reduce((sum, payment) => sum + payment.change, 0),
...
router.get('/collections/monthly', async (req, res) => {
  try {
    ...
    const dailySummary = collections.reduce((acc, payment) => {
      ...
      acc[date].byPaymentType[payment.paymentType] = (acc[date].byPaymentType[payment.paymentType] || 0) + payment.amount;
      acc[date].byPaymentMethod[payment.paymentMethod] = (acc[date].byPaymentMethod[payment.paymentMethod] || 0) + payment.amount;
      return acc;
    }, {});
...
router.get('/expenses/monthly', async (req, res) => {
  try {
    ...
    const dailySummary = expenses.reduce((acc, expense) => {
      ...
      acc[date].transactions += 1;
      acc[date].byExpenseType[expense.expenseType] = (acc[date].byExpenseType[expense.expenseType] || 0) + expense.amount;
      return acc;
    }, {});
...
router.get('/annual-summary', async (req, res) => {
  try {
    ...
    const monthlySummary = Array.from({ length: 12 }, (_, month) => {
      const monthCollections = collections.filter(p => p.createdAt.getMonth() === month);
      ...
      return {
        ...
        netIncome: monthCollections.reduce((sum, p) => sum + p.amount, 0) - 
    ...
    const summary = {
      ...
      netIncome: collections.reduce((sum, p) => sum + p.amount, 0) - 
